...
async def merge_files(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Save the current message as the previous message in user data
    context.user_data["previous_message"] = update.message

    # Fetch the last two messages sent by the user
    last_messages = [update.message, context.user_data.get("previous_message")]
...
async def merge_videos(update: Update, context: ContextTypes.DEFAULT_TYPE, messages: List[Message]) -> None:
    """Merges multiple videos into a single video."""
    user_id = update.effective_user.id
    chat_id = update.effective_chat.id

    # Download the videos
    video_paths = [await download_file(message.document.file_id, context) for message in messages]
    ...
    # Clear previous message in user data
    context.user_data["previous_message"] = None

async def merge_video_audio(update: Update, context: ContextTypes.DEFAULT_TYPE, messages: List[Message]) -> None:
    ...
    # Download the files
    video_path = await download_file(video_message.document.file_id, context)
    audio_path = await download_file(audio_message.document.file_id, context)
...
async def merge_video_subtitle(update: Update, context: ContextTypes.DEFAULT_TYPE, messages: List[Message]) -> None:
    ...
    # Download the files
    video_path = await download_file(video_message.document.file_id, context)
    subtitle_path = await download_file(subtitle_message.document.file_id, context)
